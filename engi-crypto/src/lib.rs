...
///
#[no_mangle]
pub unsafe extern "C" fn sr25519_keypair_from_secret(
    secret_key_raw: *const u8,
    keypair_raw: *mut u8,
) -> () {
    ...
    match SecretKey::from_ed25519_bytes(secret_key) {
        Ok(secret) => {
            let mut ed_bytes = secret.to_keypair().to_half_ed25519_bytes();
            let bytes = ed_bytes.as_mut_ptr();
...
pub unsafe extern "C" fn sr25519_sign(
    pub_key_raw: *const u8,
    secret_raw: *const u8,
    message_raw: *const u8,
    sz: u32,
    signature_raw: *mut u8,
) -> () {
    ...
    match (
        SecretKey::from_ed25519_bytes(secret_key),
        PublicKey::from_bytes(pub_key),
    ) {
        (Ok(s), Ok(k)) => {
            let signature = s.sign_simple(CTX, message, &k).to_bytes().to_vec();
...
pub unsafe extern "C" fn sr25519_verify(
    ...
    match (
        ...
        (Ok(s), Ok(k)) => k.verify_simple(CTX, message, &s).is_ok(),
